...
async getHiring() {
  ...
  async saveProfile() {
    ...
    await fetch('http://autoboro.markupdesigns.org/api/editAutogram',
      {
          method: 'POST',
          headers: {
    ...
    return (
      <SafeAreaView style={styles.wrapper}>
        <Header
          ...
          centerComponent={{ text: 'Edit Autogram', style: { color: '#fff', marginBottom: 25, fontSize: 16 } }}
        ...
        <ScrollView contentContainerStyle={{ minHeight: '100%' }}>
          <View>
            <LinearGradient colors={['#d55459', '#a32227', '#96151a']} style={{ height: 270, width: '100%', backgroundColor: '#FF0000' }}>
              <View >
                <Image source={this.state.avatarSource != '' ? { uri: "http://autoboro.markupdesigns.org/" + this.state.avatarSource } : splashImg} style={{ height: 180, width: '80%', alignSelf: 'center', marginTop: 30, marginBottom: 30 }}></Image>
            ...
            <View style={{ width: "90%", borderWidth: 1, marginLeft: 20, marginTop: 20 }}>
              <Textarea placeholder="Messsgae" rowSpan={5} bordered value={this.state.Description} onChangeText={(text) => this.setState({ Description: text })} style={{ backgroundColor: '#f3f3f3', borderColor: '#e8e8e8', height: 80 }} />
            </View>
